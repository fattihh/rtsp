...
# Video kaynağı
video_path = "ornek.mp4"  # Video dosyanızın yolu
...
# Doğrulama başarısızlık sayacı (tracking başladığında sıfırlanacak)
    ...
    while True:
            ...
            for box in results[0].boxes:
                ...
                if conf > 0.5 and class_name.lower() == UAV_CLASS_NAME.lower():
                    ...
                    if (bbox_abs[0] >= padding_x and bbox_abs[1] >= padding_y and 
                        ...
                        uav_bbox = bbox_abs
                        big_enough_start_time = None
                        verification_fail_count = 0  # Takip başladığında sıfırlanıyor
        ...
        # -------------------------------------------------------
            ...
            if success:
                ...
                # Aileron ve Elevator PWM hesaplaması (doğrusal ölçekleme)
                ...
                # --- THROTTLE (GAZ) PWM HESAPLAMASI ---
                # Burada throttle, bounding box alanının (pred_w * pred_h) 2000 referansına göre hesaplanıyor
                target_box_area = 2000
                current_box_area = pred_w * pred_h
                throttle_norm = 1 - (current_box_area / target_box_area)
                throttle_norm = np.clip(throttle_norm, 0, 1)
                throttle_pwm = int(1000 + throttle_norm * 1000)
                # ---------------------------------------

                # Terminale PWM değerlerini yazdırma
                print(f"Aileron: {aileron_pwm}, Elevator: {elevator_pwm}, Throttle: {throttle_pwm}")

                # Throttle göstergesi: ırtifa-yonelme çemberi ile aynı renk (40,150,255)
                throttle_circle_center = (80, frame_height - 80)
                outer_radius = 50  # Sabit dış çember yarıçapı
                cv2.circle(frame, throttle_circle_center, outer_radius, (40,150,255), 2)
                fill_angle = int(throttle_norm * 360)
                start_angle = -90
                end_angle = start_angle + fill_angle
                # Doldurulmuş yay şeklinde throttle oranını gösteriyoruz
                cv2.ellipse(frame, throttle_circle_center, (outer_radius, outer_radius), 0, start_angle, end_angle, (40,150,255), -1)
                cv2.putText(frame, f"Throttle: {throttle_pwm}", (throttle_circle_center[0] - 40, throttle_circle_center[1] - outer_radius - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (40,150,255), 2)

                # Takip edilen bbox'u KALMAN filtreden gelen tahminle çiziyoruz (Kırmızı)
                ...
                if track_frame_count % 20 == 0:
                    ...
                    if tracked_roi.size > 0:
                        ...
                        if not uav_found:
                            ...
                            print(f"Doğrulama başarısızlığı: {verification_fail_count}")
                            if verification_fail_count >= 3:
                            ...
                            verification_fail_count = 0  # Doğrulama başarılıysa sıfırla
